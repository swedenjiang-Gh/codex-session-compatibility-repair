...
工具只清空旧模型留下的非空 `reasoning` 内部内容，保留用户消息、助手可见回复、开发者消息、工具调用、工具结果和会话元数据。
...
- 仅把非空 `response_item.payload.type = "reasoning"` 的 `payload.content` 改为空数组。
- 非目标行保持字节级不变。
...
本工具解决的是 DeepSeek 等模型留下的 reasoning 内容与 GPT Responses API 不兼容的问题。若错误来自代理对工具调用、`previous_response_id` 或其他协议字段的转换，仍需代理自身修复跨模型历史迁移逻辑。
